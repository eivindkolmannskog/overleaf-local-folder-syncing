...
def upload_folder_to_dropbox(dbx: Dropbox, local_folder: str, dropbox_folder: str):
    # Get the base name of the local folder to preserve it in the Dropbox path
    base_folder_name = os.path.basename(local_folder)

    for root, dirs, files in os.walk(local_folder):
        relative_path = os.path.relpath(root, local_folder)
        # Add the base folder name to the Dropbox path
        dropbox_path = sanitize_path(dropbox_folder + "/" + base_folder_name + "/" + relative_path)
